        ...
        *,
        ...
        async def admit() -> Any:
            ...
            try:
                ...
                if registered is not None and not bool(getattr(registered, "done", False)):
                    raise AuthorityOutcomeUncertain(
    ...
    async def close(self) -> None:
        ...
        for run_id in tuple(self._waiting_executions):
            try:
                ...
                logger.warning(
                    "Waiting execution %s was not durably rejected during shutdown",
                    run_id,
                    exc_info=True,
                )
        ...
        *,
        ...
        if waiting is not None:
            ...
            result_json = self._encode_execution_result(
                AdmittedExecution(run_id, "", done=True, error=error),
                run_id,
            )
            try:
                finished = await self._coordinator.finish_command(
