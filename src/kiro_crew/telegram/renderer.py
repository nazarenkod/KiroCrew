...
#: Bytes of entropy in a per-prompt approval nonce (8 -> 16 hex chars, matching
#: Discord's). The whole ``callback_data`` is capped at 64 BYTES by Telegram and
#: carries ``a:<request_id>:<nonce>:<flag>``, so the fixed parts cost 21 and a
#: request id has 43 to spend. 64 bits is far past guessable for a value that lives
#: only as long as one prompt, and shortening it to buy request-id room would trade
#: the guarantee for headroom nothing needs.
_NONCE_BYTES = 8
...
def _display_safe(text: str) -> str:
    ...
    """
    out, _ = redact_for_display(text, redact)
    return out
...
class TelegramRenderer(Renderer):
    ...
    async def _post_thinking(self) -> None:
        ...
        if not body:
            return
        # The reasoning is model output reaching an external surface: redact
        # against the RENDERED form, since Telegram's own markup can reassemble a
        # credential that the literal bytes split.
        safe, _ = redact_for_display(body, redact)
    ...
    async def on_prompt_choice(  # noqa: D401 - imperative reads wrong for a handler
        ...
        # answer bubble don't clobber the buttons. callback_data stays under
        # Telegram's 64-byte cap (a:<request_id>:<nonce>:<1|0>, see _NONCE_BYTES
        # for the arithmetic); the callback
        # handler resolves the decider Future by reconstructing the key.
        self._note_progress()
        self._awaiting_approval = True
        rid = str(request_id)
        # Mint the per-prompt nonce and embed it in every button of THIS prompt.
        # Filed under the same key the decider awaits on, so the resolution path
        # can check that a press belongs to the prompt still on screen rather than
        # merely to a request id the provider may have reused.
        nonce = TelegramApprovalDecider.register_nonce(
            TelegramApprovalDecider.key(self._session_key, rid)
        )
        ...
        # The tool name is monospaced, so this must go out as HTML — send_message
        # defaults to plaintext and the markdown backticks would arrive literally.
        # The name is provider-supplied text, so it is escaped, and retry_plain
        # re-sends without a parse_mode if the markup is ever rejected.
        ...
        # which command it wants to run is. Already redacted by the driver, escaped
        # here, and bounded so the prompt stays one message.
