"""Channel command GRAMMAR: one spec table, matching and help rendered from it.

Deliberately separate from :mod:`kiro_crew.messaging.commands`, which owns the
channel-neutral DECISION halves of the commands themselves (``/stop``'s
cooperative-cancel contract, the ``/yolo`` ladder, dashboard-link TTLs) and states
in its own docstring that the grammar is not its business. This module is that
grammar: which words invoke a command, and the help card rendered from the same
table so the two cannot disagree. A dispatcher uses both — this one to recognise
the command, that one to carry it out.
