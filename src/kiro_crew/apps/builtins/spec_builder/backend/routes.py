...
def _refresh_slot_keys(index: dict) -> None:
    ...
    for name, meta in index.items():
        ...
        if spec_dir:
            _OBSERVED_SPEC_DIRS.setdefault(slot_key, _decision_key(spec_dir))
    ...
    for name, slot_key in _SLOT_KEYS.items():
        ...
        if not observed or observed == legacy_key:
            _OBSERVED_SLOT_KEYS[name] = slot_key
...
def _forget_observed_slot_identity(name: str, *slot_keys: str) -> None:
    ...
    for observed_name, observed_key in list(_OBSERVED_SLOT_KEYS.items()):
        if observed_key in released:
            _OBSERVED_SLOT_KEYS.pop(observed_name, None)
    for resolved_name, resolved_key in list(_SLOT_KEYS.items()):
        if resolved_key in released:
            _SLOT_KEYS.pop(resolved_name, None)
    # Keep the explicit name cleanup for a malformed empty spelling.
    if not _OBSERVED_SLOT_KEYS.get(name):
        _OBSERVED_SLOT_KEYS.pop(name, None)
    if not _SLOT_KEYS.get(name):
        _SLOT_KEYS.pop(name, None)
    for slot_key in released:
        _OBSERVED_SPEC_DIRS.pop(slot_key, None)
...
_EXECUTION_CLAIMS: dict[
    str, tuple[str, str, str, asyncio.Task[Any] | None, Any | None]
] = {}
...
def _dispatch_claim_conflicts(
    ...
    *,
    ...
    if _matching_execution_loops(
        name, normalized_dir, {slot_key}, include_orphans=True
    ):
        return True
    ...
    def _conflicts(
        ...
        return overlaps and not (
            allow_published_exact and published_slot is not None and exact
        )
...
def _reserve_pending_dispatch(dir_key: str, slot_key: str, name: str) -> str:
    ...
    if _EXECUTION_STOPS.get(name, 0):
        return ""
    if _dispatch_claim_conflicts(
        dir_key, slot_key, name, allow_published_exact=True
    ):
        return ""
...
def _release_pending_dispatch_when_done(token: str) -> None:
    ...
    if task is not None:
        task.add_done_callback(
            lambda done: _drop_pending_dispatch_if_owner(token, done)
        )


def _bind_pending_dispatch_to_turn(
    token: str, slot: Any, turn: asyncio.Task[Any] | None
) -> None:
...
def _drop_execution_claim_if_owner(
    dir_key: str, token: str, owner: asyncio.Task[Any]
) -> None:
...
async def _execution_stop_barrier(
    ...
    claimed_executions: dict[
        str, tuple[str, str, str, asyncio.Task[Any] | None, Any | None]
    ] = {}
    claimed_pending: dict[
        str, tuple[str, str, str, asyncio.Task[Any] | None, Any | None]
    ] = {}
    normalized_dir = _decision_key(dir_key)
    ...
    for claim_dir, claim in list(
        _EXECUTION_CLAIMS.items()
    ):
        token, claim_slot_key, claim_name, _owner, _slot = claim
        if (
            claim_dir == normalized_dir
            or claim_slot_key == slot_key
            or claim_name == name
        ):
    ...
    for token, (claim_dir, claim_slot_key, claim_name, _owner, _slot) in list(
        ...
        if (
            ...
            claimed_slots.setdefault(
                claim_slot_key, _exec_loop_id_for_slot(claim_slot_key)
            )
    ...
    for observed_slot_key in _observed_slot_keys_for_dir(normalized_dir):
        claimed_slots.setdefault(
            observed_slot_key, _exec_loop_id_for_slot(observed_slot_key)
        )
    ...
    for orphaned_slot_key in _unindexed_observed_slot_keys():
        claimed_slots.setdefault(
            orphaned_slot_key, _exec_loop_id_for_slot(orphaned_slot_key)
        )
...
async def _pin_legacy_slot_identity(name: str, meta: dict) -> dict | None:
    ...
    if isinstance(persisted, str) and persisted:
        return (
            meta
            if _owns_slot_key(name, persisted)
            and (not observed or persisted == observed)
            else None
        )
    legacy_key = f"spec-builder-{name}"
    ...
    *,
    ...
    for loop in loops:
        ...
        belongs_to_index = any(
            ...
            for indexed_name in _INDEXED_SPEC_NAMES
...
async def _remove_orphaned_executions_with_service(state: Any, service: Any) -> set[str]:
    ...
    orphaned_loops = _matching_execution_loops(
        "", "", set(), include_orphans=True, service=service
    )
    ...
    _forget_observed_slot_identity(
        "", *(orphaned & _unindexed_observed_slot_keys())
    )
    return orphaned
...
async def _remove_nudge_loop_for_slot(
    slot_key: str, *, only_loop_id: Any = _UNPINNED
) -> None:
...
def _busy_observed_directory_slot(
    state: Any, dir_key: str, own_slot_key: str
) -> str:
...
async def _final_alias_conflict(
    ...
    *,
    ...
    """
    fresh_aliases = await _alias_slots(
        dir_key, own_slot_key=own_slot_key, own_name=own_name
    )
...
async def _handle_create(request: web.Request) -> web.Response:
    ...
    async with _turn_lock(create_dir_key):
        ...
        creation_dispatch_claim = _reserve_pending_dispatch(
            str(spec_dir), slot_key, name
        )
    ...
    if handler_task is not None:
        ...
        def _release_abandoned_claim(_done: asyncio.Task[Any]) -> None:
            _drop_execution_claim_if_owner(
                handoff_dir_key, execution_claim, _done
            )
    ...
    async with _turn_lock(handoff_dir_key):
        ...
        if (
            ...
            return web.json_response(
                {
                    "code": (
                        "execution_stopped_during_start"
                        if stopped
                        else "spec_changed_during_start"
                    ),
                    "error": (
                        "execution was stopped before it started"
                        if stopped
        ...
        _bind_execution_claim_to_turn(
            handoff_dir_key, execution_claim, slot, turn
        )
...
async def _handle_stop_execution(request: web.Request) -> web.Response:
    ...
    async with _execution_stop_barrier(
        dir_key, original_slot_key, name
    ) as claimed_slot_keys, _turn_lock(dir_key):
        ...
        try:
            ...
            for claimed_slot_key, claimed_loop_id in claimed_slot_keys.items():
                if (
                    claimed_slot_key == captured_slot_key
                    and claimed_loop_id == captured_loop_id
                ):
                    continue
                await _remove_nudge_loop_for_slot(
                    claimed_slot_key, only_loop_id=claimed_loop_id
                )
...
async def _handle_delete(request: web.Request) -> web.Response:
    ...
    async with _execution_stop_barrier(
        doomed_key, doomed_runtime_slot_key, name
    ) as claimed_slot_keys, _turn_lock(doomed_key):
        ...
        def _pop_if_same(idx: dict) -> bool:
            ...
            actual_key = str(meta.get("slot_key", ""))
            if (
                doomed_commit_slot_key
                and actual_key
                and actual_key != doomed_commit_slot_key
            ):
                return False
        ...
        try:
            popped = await _mutate_index(
                _pop_if_same,
                on_commit=lambda: _forget_observed_slot_identity(
                    name, *released_slot_keys
                ),
            )
        except Exception:
