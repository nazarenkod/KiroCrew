...
class WebexRenderer(Renderer):
    ...
    async def on_thinking(self, text: str) -> None:
        # Webex does not surface reasoning inline: it would cost edits the answer
        # needs, and there is no collapsible affordance to hide it behind.
        return None
    ...
    async def on_prompt_choice(self, options: list[dict[str, Any]], request_id: str | int) -> None:
        ...
        ``WebexDispatcher``.
        """
        tool = _safe_tool_label(self._tool) or "this tool"
        rid = str(request_id)
    ...
    async def on_done(self, stop_reason: str = "") -> None:
        ...
        if card is None:
            ...
            body = append_options_text(body, choices)
            kept = []
        ...
        for chunk in rest:
            ...
            if sent is None:
                ...
                # of the answer. Break rather than return — the uploads and the
                # card below belong to the prefix that DID land, and the card in
                # particular holds the only copy of the kept choices.
        ...
        if card is not None:
            ...
            sent = await self._client.send_message(
                self._room_id,
                append_options_text(_CHOICE_PROMPT, kept),
                attachments=[card],
            ...
            if sent is None:
                ...
                await self._client.send_message(
                    self._room_id,
                    append_options_text("", kept).lstrip("\n"),
                    parent_id=self._thread_id,
                )
    ...
    def text(self) -> str:
        ...
        """
        return split_options_trailer("".join(self._buf).strip())[0]
    ...
    async def _send_uploads(self, files: list[Any]) -> list[Any]:
        ...
        three follow-up messages. The bytes come from the extractor; the path is
        used only for the filename it puts on the upload.
        ...
        for item in files:
            sent = await self._client.send_file(
                self._room_id,
                item.alt or os.path.basename(item.path),
                data=item.data,
            ...
            if sent is None:
                logger.warning("Webex: upload failed for %s", os.path.basename(item.path))
                failed.append(item)
        return failed
