...
class WebexDispatcher:
    ...
    async def handle_message(
        ...
        *,
        ...
        if _APPROVALS.has_pending(session_key):
        ...
        decider = (
            SessionApprovalDecider(_APPROVALS, session_key=session_key)
        ...
        renderer = WebexRenderer(
            ...
            mint_approval_nonce=lambda rid: _APPROVALS.reserve(session_key, rid),
            publish_choices=lambda nonce, choices: self._choices.publish(
                session_key, nonce, choices
            ),
        )
        ...
        try:
            ...
            _APPROVALS.discard_reservations(session_key)
            if temp_paths:
    ...
    async def _maybe_answer_approval(
        ...
        if approved and not permitted:
            ...
            return True
        resolved = _APPROVALS.resolve(session_key, approved)
        sel().log_api_access(
    ...
    async def _handle_card_press(self, inbound: "WebexInbound", *, permitted: bool) -> None:
        ...
        if kind == cards.KIND_OPTIONS:
            ...
            if not permitted:
                ...
                return
            text = self._choices.take(session_key, choice, nonce)
            if not text:
    ...
    async def _resolve_card_approval(
        ...
        if not nonce or not request_id:
            ...
            resolved = _APPROVALS.resolve(
                session_key, approved, request_id=request_id, expected_nonce=nonce
            )
    ...
    async def _drain_queue(self, inbound: "WebexInbound", session_key: str) -> None:
        ...
        while True:
            ...
            drained = replace(inbound, text="\n\n".join(texts), file_urls=tuple(files))
    ...
    async def _handle_link(self, inbound: "WebexInbound") -> None:
        ...
        # One write for the whole sequence: each mutation would otherwise rewrite
        # the entire session map, stalling the loop three times for what is one
        # user-visible action.
        with self.sessions.batched_save():
            ...
            # rebind (reads prefer the channel key, but a leftover row would
            # still answer a clear).
