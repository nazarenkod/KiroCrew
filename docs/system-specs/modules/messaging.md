...
| `max_message_chars` | `4096` | quantitative — Slack 3900, Telegram 4096, Discord 2000, Teams 16000, WhatsApp 4096. A CHARACTER count: a byte-capped platform must declare a value safe at its worst-case bytes-per-char (Webex, Teams and WeCom are pinned in `test_capability_ledger.py`) |
...
via the shared `render_options_as_text`) rather than being deleted; interactive
tool approvals run decider-less, so under INTERACTIVE mode the only rung that can
