...
is invoked exactly from the raw `EVENT_COMPLETE` branch, before `DONE`, with the
event's `TurnUsage` and a disposition derived from its stop reason. Completion
accounting therefore survives cancellation during renderer finalization. A normal
...
and legacy AutoNudge turns. Directive consumption occurs
