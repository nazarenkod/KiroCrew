...
Within that block, each document's own front matter decides whether it loads (`context.py:_steering_inclusion_skips`, parsed through the `STEERING_LOADER` dialect in `frontmatter.py`). `inclusion: manual` is honored and the document is **withheld**: the author declared it loads only when explicitly referenced, so injecting it into every session contradicts the one instruction the file gives about itself. `inclusion: fileMatch` is deliberately **not** honored — nothing on this path evaluates `fileMatchPattern`, and withholding the document would mean it never applies at all, a silent capability loss, where unconditional injection is merely the coarse behavior this path already had. An absent or unrecognized value is treated as `always` (Kiro's documented default) so a typo cannot quietly drop a document from every session. Skips are counted in the loader's debug line, because a document withheld on its own declaration is otherwise indistinguishable at the prompt from one that failed to load. Conditional loading by pattern, and an explicit-reference path for `manual`, are tracked as separate work.

Note this whole path is **dormant in the public fork**: `provider_type` reaches `build_message()` as `cfg.agent.provider`, which the config schema pins to `enum=["acp"]`, and the one caller that derives it from a live client (`subagent.py:_provider_label_of`) can only answer `claude_code` for `ACP_BACKEND_CLAUDE` — a seam excluded from `ACP_BACKENDS_SELECTABLE`. The gate is therefore correctness work on the dormant seam, not a live user-facing behavior change.
