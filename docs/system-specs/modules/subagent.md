...
acceptance for success. The legacy lost-wave endpoint remains for unkeyed
compatibility callers. The
...
3. A recorded child is killed only when the active fenced executor persisted
   ...
   identity-pinned process handle through termination. Every successful recovery
   termination emits the same `orphan_reconcile_kill` SEL audit as the
