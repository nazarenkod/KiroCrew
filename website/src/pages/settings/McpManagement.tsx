    ...
    <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--card)]">
        ...
        <div className="border-t border-[var(--border)] px-4 py-3 text-[12.5px] leading-relaxed text-[var(--muted)]">
          ...
          {/* This view renders the same state chips through the same derivation, so
              a term defined only under the OTHER tab is undecodable to the operator
              auditing sharing here -- which is this fix's whole audience. Defined
              once, in its own key, and rendered wherever the chip can appear: a
              second copy in this view's own legend string would be two catalogs
              that must agree about one state in 13 locales, which is the drift this
    ...
    <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--card)]">
        ...
        <div className="border-t border-[var(--border)] px-4 py-3 text-[12.5px] leading-relaxed text-[var(--muted)]">
          ...
          {/* On its own line rather than spliced into the paragraph above. The
              paragraph is one catalog string defining four pre-existing terms, and
              editing it per locale meant re-deriving a spliced sentence in 13 files
              on every wording change; as a separate element the new term is also
              findable by scanning instead of by reading the whole paragraph. */}
          <span className="mt-2 block">
            {i18nT('pages.mcpManagement.state_direct_env_legend')}
          </span>
        </div>
