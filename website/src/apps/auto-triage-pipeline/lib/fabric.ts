...
export function laneTimelineRows(lane: FabricLane): TimelineRow[] {
  const rows = lane.timeline.map((entry, k) => {
  ...
  // The payload's `timeline` is a SPINE timeline -- the backend keeps off-spine steps
  // out of it deliberately and its tests say so. But this table is presented as the
  // item's phase history, so stopping at the last on-spine phase made it contradict
  // the lane beside it: an item that ended in `awaiting-reply` showed a stub and a
  // chip both saying AWAIT REPLY, over a table that never mentioned it. The exit is
  // carried separately, so append it as the closing row.
  if (lane.exit) {
    const prev = lane.timeline.length ? lane.timeline[lane.timeline.length - 1].atMs : null
    const secs = dwellSeconds(prev, lane.exit.atMs)
    rows.push({
      phase: lane.exit.phase,
      atMs: lane.exit.atMs,
      dwellSeconds: secs,
      slow: secs != null && secs > SLOW_DWELL_SECONDS,
    })
